<script type="text/javascript">
	...
	function peticionAJAX(nombre, id_profesion) {
	//xhr = new XMLHttpRequest();
	xhr.open("POST", "<?=base_url()?>profesion/editarPost", true);
	xhr.setRequestHeader("X-Requested-With", "XMLHttpRequest");
	xhr.setRequestHeader("Content-type","application/x-www-form-urlencoded");
	console.log(nombre);
	xhr.send("nombre="+nombre+"&id_profesion="+id_profesion);
	console.log(xhr);
	...
	<form id="idFormulario" method="post" action="<?=base_url()?>profesion/editarPost">
		<fieldset>
			...
			<div class="form-group">
				...
				<input class="form-control" type="hidden" id="idID" name="id_profesion"
				<?= "value=".$body["profesiones"]->id?>>
